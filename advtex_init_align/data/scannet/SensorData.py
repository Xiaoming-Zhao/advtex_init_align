...
class RGBDFrame():
  ...
  def load(self, file_handle):
    ...
    # https://github.com/ScanNet/ScanNet/issues/46#issuecomment-855159784
...
class SensorData:
  ...
  def export_depth_images(self, output_path, image_size=None, frame_skip=1):
    ...
    for f in tqdm.tqdm(range(0, len(self.frames), frame_skip)):
      ...
      with open(os.path.join(output_path, f"{f:06d}.png"), 'wb') as f: # write 16-bit
  ...
  def export_color_images(self, output_path, image_size=None, frame_skip=1):
    ...
    for f in tqdm.tqdm(range(0, len(self.frames), frame_skip)):
      ...
      imageio.imwrite(os.path.join(output_path, f"{f:06d}.jpg"), color)
  ...
  def export_poses(self, output_path, frame_skip=1):
    ...
    for f in tqdm.tqdm(range(0, len(self.frames), frame_skip)):
      ...
      self.save_mat_to_file(self.frames[f].camera_to_world, os.path.join(output_path, f"{f:06d}.txt"))
  ...
  def export_intrinsics(self, output_path):
    ...
    # NOTE: check whether RGB and depth are aligned
    assert np.all(self.extrinsic_color == np.eye(4)), f"{self.extrinsic_color}"
    assert np.all(self.extrinsic_depth == np.eye(4)), f"{self.extrinsic_depth}"
